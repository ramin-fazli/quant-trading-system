...
from data.mt5 import MT5DataManager, OptimizedPairsStrategy
...
class VectorBTBacktester:
    ...
    def __init__(self, config, data_manager):
        ...
        # Dynamically select or require a strategy engine
        # If data_manager has an 'strategy_class', use it, else try to import OptimizedPairsStrategy
        if hasattr(data_manager, 'strategy_class'):
            self.strategy = data_manager.strategy_class(config, data_manager)
        else:
            try:
                from data.mt5 import OptimizedPairsStrategy
                self.strategy = OptimizedPairsStrategy(config, data_manager)
            except ImportError:
                raise RuntimeError("No compatible strategy class found for the provided data manager.")
    ...
    def _calculate_trading_costs(self, s1: str, s2: str, data: pd.DataFrame) -> pd.Series:
        """Calculate trading costs for each timestamp (data manager agnostic)"""
        # Try to get symbol info from data_manager, fallback to default values
        info1 = {}
        info2 = {}
        # Try MT5 style
        ...
        # Try cTrader style
            ...
            info1 = self.data_manager.symbol_details.get(s1, {})
            info2 = self.data_manager.symbol_details.get(s2, {})

        # Calculate spread costs
        spread1 = info1.get('spread', 2)
        point1 = info1.get('point', 0.0001)
        spread2 = info2.get('spread', 2)
        point2 = info2.get('point', 0.0001)
        spread1_pct = spread1 * point1 / data['price1'] * 100 if 'price1' in data else 0.1
        spread2_pct = spread2 * point2 / data['price2'] * 100 if 'price2' in data else 0.1

        # Commission logic (can be extended for cTrader)
        def is_stock_or_etf(symbol: str) -> bool:
            # Try to detect from info dict or symbol name
            if 'asset_class' in info1 and info1['asset_class'] in ('stock', 'etf'):
                return True
            if symbol.endswith('.ST') or symbol.endswith('.ETF'):
                return True
            return False
        ...
        if is_stock_or_etf(s1):
            commission1_pct += getattr(self.config, 'commission_fixed', 0.02) / data['price1'] * 100
        if is_stock_or_etf(s2):
            commission2_pct += getattr(self.config, 'commission_fixed', 0.02) / data['price2'] * 100

        # Total costs (round trip for both legs)
        total_costs = (2 * commission1_pct + 2 * commission2_pct + spread1_pct + spread2_pct)
