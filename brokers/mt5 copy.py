...
class MT5RealTimeTrader:
    ...
    def __init__(self, config: TradingConfig, data_manager: MT5DataManager):
        ...
        if account_info is not None:
            ...
            self.initial_portfolio_value = getattr(account_info, 'equity', config.initial_portfolio_value)
            self.portfolio_peak_value = self.initial_portfolio_value
        else:
            self.account_currency = 'USD'
            self.initial_portfolio_value = config.initial_portfolio_value
            self.portfolio_peak_value = self.initial_portfolio_value
    ...
    def _calculate_portfolio_current_value(self) -> float:
        """Calculate current portfolio value including unrealized P&L, using MT5 equity as base."""
        account_info = mt5.account_info()
        if account_info is not None:
            current_value = getattr(account_info, 'equity', self.initial_portfolio_value)
        else:
        ...
        for pair_str, position in self.active_positions.items():
            try:
                ...
                if position['direction'] == 'LONG':
                    pnl1 = current_value1 - entry_value1
                    pnl2 = entry_value2 - current_value2
                else:
                    pnl1 = entry_value1 - current_value1
                    pnl2 = current_value2 - entry_value2

                # Convert P&L to account currency if needed
                cur1 = self._get_symbol_currency(position['symbol1'])
                cur2 = self._get_symbol_currency(position['symbol2'])
                fx1 = self._get_fx_rate(cur1, self.account_currency)
                fx2 = self._get_fx_rate(cur2, self.account_currency)
                pnl1_ccy = pnl1 * fx1
                pnl2_ccy = pnl2 * fx2

                current_value += pnl1_ccy + pnl2_ccy
    ...
    def _calculate_position_pnl(self, pair_str: str) -> float:
        """Calculate the current P&L for a given pair position in account currency."""
        ...
        try:
            ...
            if position['direction'] == 'LONG':
                pnl1 = close_value1 - entry_value1
                pnl2 = entry_value2 - close_value2
            else:
                pnl1 = entry_value1 - close_value1
                pnl2 = close_value2 - entry_value2

            # Convert P&L to account currency if needed
            cur1 = self._get_symbol_currency(s1)
            cur2 = self._get_symbol_currency(s2)
            fx1 = self._get_fx_rate(cur1, self.account_currency)
            fx2 = self._get_fx_rate(cur2, self.account_currency)
            pnl1_ccy = pnl1 * fx1
            pnl2_ccy = pnl2 * fx2

            return pnl1_ccy + pnl2_ccy
    ...
    def initialize(self) -> bool:
        ...
        # Update initial portfolio value and peak from MT5
        account_info = mt5.account_info()
        if account_info is not None:
            self.account_currency = getattr(account_info, 'currency', 'USD')
            self.initial_portfolio_value = getattr(account_info, 'equity', self.initial_portfolio_value)
            self.portfolio_peak_value = self.initial_portfolio_value
                ...
                while self.is_trading:
                    ...
                    if status_counter % 300 == 0:
                        ...
                        if self.data_manager._ensure_connection():
                            account_info = mt5.account_info()
                            logger.info(f"Account equity: {account_info.equity if account_info else 'N/A'}")
                        else:
                            logger.warning("Could not retrieve account equity due to connection issue.")
                        logger.info("==============================")
            ...
            try:
                ...
                if loop_count % 1 == 0:
                    try:
                        ...
                        logger.info(f"Current Equity    : €{current_portfolio_value:,.2f}")
                        ...
                        realized_pnl = current_portfolio_value - self.initial_portfolio_value - total_open_pnl
                        realized_pnl_pct = (realized_pnl / self.initial_portfolio_value * 100)
    ...
    def _calculate_position_pnl_pct(self, pair_str: str) -> float:
        """Calculate the PnL percentage for a given pair position in account currency."""
        if pair_str not in self.active_positions:
            return 0.0
        ...
        total_position_value = (abs(entry_value1 * fx1) + abs(entry_value2 * fx2)) / 2
        if total_position_value == 0:
            return 0.0

        pnl_pct = ((pnl1_ccy + pnl2_ccy) / total_position_value) * 100
        return pnl_pct
