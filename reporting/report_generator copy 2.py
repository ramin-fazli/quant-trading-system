...
def _add_enhanced_trade_details(sheet, result, formats, safe_write_numeric, start_row):
    """Add enhanced trade details with better formatting"""
    trades = result.get('trades', [])
    if not trades:
        return
    ...
    sheet.write(trade_start_row, trade_start_column, 'TRADE HISTORY', formats['section_title'])
    sheet.merge_range(f'{chr(65 + trade_start_column)}{trade_start_row+1}:{chr(65 + trade_start_column + 11)}{trade_start_row+1}', 'TRADE HISTORY', formats['section_title'])
    
    headers = [
        'Trade #', 'Entry Time', 'Exit Time', 'Direction', 'Entry Price', 
        'Exit Price', 'Gross PnL (%)', 'Net PnL (%)', 'Costs (%)', 'Exit Reason', 
        'Bars Held', 'Cumulative PnL (%)'
    ]
    ...
    for row_idx, trade in enumerate(trades[:100], trade_start_row + 3):  # Limit to first 100 trades
        ...
        safe_write_numeric(sheet, row_idx, trade_start_column + 4, trade.get('entry_price', 0), formats['number'])  # Entry Price
        safe_write_numeric(sheet, row_idx, trade_start_column + 5, trade.get('exit_price', 0), formats['number'])  # Exit Price
        
        # PnL with color coding
        gross_pnl = trade.get('pnl_pct', 0)
        gross_fmt = formats['positive'] if gross_pnl > 0 else formats['negative']
        safe_write_numeric(sheet, row_idx, trade_start_column + 6, gross_pnl/100, formats['percentage'])  # Gross PnL
        
        net_pnl = trade.get('net_pnl_pct', 0)
        net_fmt = formats['positive'] if net_pnl > 0 else formats['negative']
        safe_write_numeric(sheet, row_idx, trade_start_column + 7, net_pnl/100, formats['percentage'])  # Net PnL
        
        # Costs
        costs = trade.get('costs_pct', 0)
        safe_write_numeric(sheet, row_idx, trade_start_column + 8, costs/100, formats['percentage'])  # Costs
        
        # Exit reason
        sheet.write(row_idx, trade_start_column + 9, trade.get('exit_reason', ''), formats['text'])  # Exit Reason
        
        # Bars held
        safe_write_numeric(sheet, row_idx, trade_start_column + 10, trade.get('bars_held', 0), formats['integer'])  # Bars Held
        
        # Cumulative PnL
        cumulative_pnl += net_pnl
        cum_fmt = formats['positive'] if cumulative_pnl > 0 else formats['negative']
        safe_write_numeric(sheet, row_idx, trade_start_column + 11, cumulative_pnl/100, formats['percentage'])  # Cumulative PnL
    ...
    if len(trades) > 0:
        ...
        # Gross PnL (column 6 from start = trade_start_column + 6)
        sheet.conditional_format(trade_start_row + 3, trade_start_column + 6, last_trade_row, trade_start_column + 6, {
            'type': '3_color_scale',
            'min_color': '#FF6B6B',
            'mid_color': '#FFFFFF',
            'max_color': '#4ECDC4'
        ...
        # Net PnL (column 7 from start = trade_start_column + 7)
        sheet.conditional_format(trade_start_row + 3, trade_start_column + 7, last_trade_row, trade_start_column + 7, {
            'type': '3_color_scale',
            'min_color': '#FF6B6B',
            'mid_color': '#FFFFFF',
            'max_color': '#4ECDC4'
        ...
        # Cumulative PnL (column 11 from start = trade_start_column + 11)
